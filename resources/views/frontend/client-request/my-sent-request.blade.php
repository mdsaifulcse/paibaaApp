...
@section('title') My Sent Request | Khojlei Paibaa | Shob Paibaa  @endsection
        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">My Sent Request</li>
                </ol>
            </nav>
        </div>
    </div>
                            ...
                            <div id="recent-transactions" class="col-12"> <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                                ...
                                <div class="table table-responsive">
                                    <table class="table table-xl table-bordered mb-0 table-responsive">
                                        ...
                                        <tbody>
                                        @if(count($mySentRequests)>0)
                                        @foreach($mySentRequests as $key =>$data)
                                        <tr>
                                            <td class="text-truncate">{{$key+1}}</td>
                                            <td class="text-truncate"><a href="{{url('/ad/'.$data->priceNegotiationOfAds->link.'?user='.encrypt($data->request_to).'&offer='.encrypt($data->offer))}}">{{$data->request_message}}</a>
                                            </td>

                                            <td class="text-truncate">
                                                <a href="{{url('/ad/'.$data->priceNegotiationOfAds->link.'?user='.encrypt($data->request_to).'&offer='.encrypt($data->offer))}}"><p>{{$data->priceNegotiationOfAds->title}}</p></a>
                                                </td>
                                            ...
                                            <td class="text-truncate">
                                                <div class="form-check">
                                                    ...
                                                    <div class="recent_img" style="width: 60px;">
                                                        <a href="{{url('/ad/'.$data->priceNegotiationOfAds->link.'?user='.encrypt($data->request_to).'&offer='.encrypt($data->offer))}}">
                                                            @if(!empty($data->replayUser->image))
                                                                <img class="img-fluid rounded-top" src="{{asset($data->replayUser->image)}}" alt="{{$data->replayUser->name}}" title="{{$data->replayUser->name}}">

                                                            @else
                                                                <img class="img-fluid rounded-top" src="{{asset('/images/default/photo.png')}}" alt="{{$data->replayUser->name}}" title="{{$data->replayUser->name}}">
                                                            @endif

                                                            <span>{{$data->replayUser->name}}</span>
                                                        </a>
                                                    </div>
                                                </div>
                                            </td>
